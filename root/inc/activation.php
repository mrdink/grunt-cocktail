...
function {%= prefix %}_theme_activation_options_init() {
	register_setting(
		'{%= prefix %}_activation_options',
		'{%= prefix %}_theme_activation_options'
	);
}
add_action('admin_init', '{%= prefix %}_theme_activation_options_init');

function {%= prefix %}_activation_options_page_capability() {
	return 'edit_theme_options';
}
add_filter('option_page_capability_{%= prefix %}_activation_options', '{%= prefix %}_activation_options_page_capability');

function {%= prefix %}_theme_activation_options_add_page() {
	${%= prefix %}_activation_options = {%= prefix %}_get_theme_activation_options();

	if (!${%= prefix %}_activation_options) {
		add_theme_page(
			__('Theme Activation', '{%= prefix %}'),
			__('Theme Activation', '{%= prefix %}'),
			'edit_theme_options',
			'theme_activation_options',
			'{%= prefix %}_theme_activation_options_render_page'
		);
	} else {
...
add_action('admin_menu', '{%= prefix %}_theme_activation_options_add_page', 50);

function {%= prefix %}_get_theme_activation_options() {
	return get_option('{%= prefix %}_theme_activation_options');
}

function {%= prefix %}_theme_activation_options_render_page() { ?>
	<div class="wrap">
		<h2><?php printf(__('%s Theme Activation', '{%= prefix %}'), wp_get_theme()); ?></h2>
		<div class="updated">
			<?php _e('<p>These settings are optional and should usually be used only on a fresh installation.</p>', '{%= prefix %}'); ?>
		</div>

		<?php settings_errors(); ?>

		<form method="post" action="options.php">
			<?php settings_fields('{%= prefix %}_activation_options'); ?>
			<table class="form-table">
				<tr valign="top"><th scope="row"><?php _e('Create a style guide?', '{%= prefix %}'); ?></th>
					<td>
						<fieldset>
							<legend class="screen-reader-text"><span><?php _e('Create a style guide?', '{%= prefix %}'); ?></span></legend>
							<select name="{%= prefix %}_theme_activation_options[create_style_guide]" id="create_style_guide">
								<option selected="selected" value="true"><?php echo _e('Yes', '{%= prefix %}'); ?></option>
								<option value="false"><?php echo _e('No', '{%= prefix %}'); ?></option>
							</select>
							<p class="description"><?php printf(__('Create a page called Style Guide to show theme styles', '{%= prefix %}')); ?></p>
						</fieldset>
					</td>
				</tr>
				<tr valign="top"><th scope="row"><?php _e('Change permalink structure?', '{%= prefix %}'); ?></th>
					<td>
						<fieldset>
							<legend class="screen-reader-text"><span><?php _e('Update permalink structure?', '{%= prefix %}'); ?></span></legend>
							<select name="{%= prefix %}_theme_activation_options[change_permalink_structure]" id="change_permalink_structure">
								<option selected="selected" value="true"><?php echo _e('Yes', '{%= prefix %}'); ?></option>
								<option value="false"><?php echo _e('No', '{%= prefix %}'); ?></option>
							</select>
							<p class="description"><?php printf(__('Change permalink structure to /&#37;postname&#37;/', '{%= prefix %}')); ?></p>
						</fieldset>
					</td>
				</tr>
				<tr valign="top"><th scope="row"><?php _e('Create navigation menu?', '{%= prefix %}'); ?></th>
					<td>
						<fieldset>
							<legend class="screen-reader-text"><span><?php _e('Create navigation menu?', '{%= prefix %}'); ?></span></legend>
							<select name="{%= prefix %}_theme_activation_options[create_navigation_menus]" id="create_navigation_menus">
								<option selected="selected" value="true"><?php echo _e('Yes', '{%= prefix %}'); ?></option>
								<option value="false"><?php echo _e('No', '{%= prefix %}'); ?></option>
							</select>
							<p class="description"><?php printf(__('Create the Primary Navigation menu and set the location', '{%= prefix %}')); ?></p>
						</fieldset>
					</td>
				</tr>
				<tr valign="top"><th scope="row"><?php _e('Add pages to menu?', '{%= prefix %}'); ?></th>
					<td>
						<fieldset>
							<legend class="screen-reader-text"><span><?php _e('Add pages to menu?', '{%= prefix %}'); ?></span></legend>
							<select name="{%= prefix %}_theme_activation_options[add_pages_to_primary_navigation]" id="add_pages_to_primary_navigation">
								<option selected="selected" value="true"><?php echo _e('Yes', '{%= prefix %}'); ?></option>
								<option value="false"><?php echo _e('No', '{%= prefix %}'); ?></option>
							</select>
							<p class="description"><?php printf(__('Add all current published pages to the Primary Navigation', '{%= prefix %}')); ?></p>
						</fieldset>
					</td>
				</tr>
			</table>
...
function {%= prefix %}_theme_activation_action() {
	if (!(${%= prefix %}_theme_activation_options = {%= prefix %}_get_theme_activation_options())) {
		return;
	}
	...
	if (${%= prefix %}_theme_activation_options['create_style_guide'] === 'true') {
		${%= prefix %}_theme_activation_options['create_style_guide'] = false;

		$default_pages = array(__('Style Guide', '{%= prefix %}'));
...
http://youtu.be/OTRmyXX6ipU',
		...
		$guide = get_page_by_title(__('Style Guide', '{%= prefix %}'));
	...
	if (${%= prefix %}_theme_activation_options['change_permalink_structure'] === 'true') {
		${%= prefix %}_theme_activation_options['change_permalink_structure'] = false;
	...
	if (${%= prefix %}_theme_activation_options['create_navigation_menus'] === 'true') {
		${%= prefix %}_theme_activation_options['create_navigation_menus'] = false;

		${%= prefix %}_nav_theme_mod = false;

		$primary_nav = wp_get_nav_menu_object(__('Primary Navigation', '{%= prefix %}'));

		if (!$primary_nav) {
			$primary_nav_id = wp_create_nav_menu(__('Primary Navigation', '{%= prefix %}'), array('slug' => 'primary_navigation'));
			${%= prefix %}_nav_theme_mod['primary_navigation'] = $primary_nav_id;
		} else {
			${%= prefix %}_nav_theme_mod['primary_navigation'] = $primary_nav->term_id;
		}

		if (${%= prefix %}_nav_theme_mod) {
			set_theme_mod('nav_menu_locations', ${%= prefix %}_nav_theme_mod);
		}
	}

	if (${%= prefix %}_theme_activation_options['add_pages_to_primary_navigation'] === 'true') {
		${%= prefix %}_theme_activation_options['add_pages_to_primary_navigation'] = false;
	...
	update_option('{%= prefix %}_theme_activation_options', ${%= prefix %}_theme_activation_options);
}
add_action('admin_init','{%= prefix %}_theme_activation_action');

function {%= prefix %}_deactivation() {
	delete_option('{%= prefix %}_theme_activation_options');
}
add_action('switch_theme', '{%= prefix %}_deactivation');
